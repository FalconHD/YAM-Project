...
import { putTransPost } from '../../../actions/CovoiturageActions'
...
onSubmit = (e,props) => {
  ...
  const Post =  { lieu_depart, lieu_arrive, date_depart, date_arrive, num_tele ,ville,bagages,comment,lieu,distination,num_places};
  this.props.putTransPost(this.props.values.id,Post);
...
componentDidMount(){
  ...
  render() {
    const {
      values: { lieu_depart, lieu_arrive, date_depart, date_arrive, num_tele ,ville,bagages,comment,lieu,distination,num_places}} = this.props;
    return (
      ...
      <div className="ConfirmWithSub">
        <div className="resualt-list">
              ...
              <div><h6>Destination Place"</h6><span>{distination}</span></div>
...
export default connect(mapStateToProps ,{getProfile , putTransPost})(ConfirmO);
